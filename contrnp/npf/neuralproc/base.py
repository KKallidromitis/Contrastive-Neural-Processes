...
class NeuralProcessFamily(nn.Module, abc.ABC):
    ...
    def __init__(
        ...
        self.image_contrnp = image_contrnp
    ...
    def forward(self, X_cntxt, Y_cntxt, X_trgt, Y_trgt=None,return_encoding=False):
        """
        ...
        #print(X_cntxt.shape, Y_cntxt.shape)
        #print(X_cntxt.shape,Y_cntxt.shape)
        ...
        #print('NN',R_trgt.shape,X_trgt.shape,R.shape)
        if self.encoded_path == "latent":
            R=R_latent

        if return_encoding:
            
            if self.image_contrnp:
                return torch.mean(torch.mean(R,dim=1),dim=1)
            else:
        ...
        if self.is_contrastive:
            ...
            if self.image_contrnp:
                #print('R',R.shape)
                return p_yCc, z_samples, q_zCc, q_zCct,torch.mean(torch.mean(R,dim=1),dim=1)
            else:
                return p_yCc, z_samples, q_zCc, q_zCct, torch.mean(R, dim=1)
    ...
    def decode(self, X_trgt, R_trgt):
        ...
        #print('Decoder',X_trgt.shape, R_trgt.shape)
